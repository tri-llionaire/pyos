...
try:
    ...
    n.write('[{:.8f}] boot: imports.check loaded\n'.format(time.time() - starttime))
    print('[{:.8f}] boot: loading hashing'.format(time.time() - starttime))
    from imports import hashing
    n.write('[{:.8f}] boot: imports.hashing loaded\n'.format(time.time() - starttime))
    print('[{:.8f}] boot: loading scan'.format(time.time() - starttime))
    from imports import scan
    n.write('[{:.8f}] boot: imports.scan loaded\n'.format(time.time() - starttime))
    print('[{:.8f}] boot: loading cotw'.format(time.time() - starttime))
    from imports import cotw
    n.write('[{:.8f}] boot: imports.cotw loaded\n'.format(time.time() - starttime))
    print('[{:.8f}] boot: loading stock'.format(time.time() - starttime))
    from imports import stock
    n.write('[{:.8f}] boot: imports.stock loaded\n'.format(time.time() - starttime))
    print('[{:.8f}] boot: loading clocking'.format(time.time() - starttime))
    from imports import clock
    n.write('[{:.8f}] boot: imports.clock loaded\n'.format(time.time() - starttime))
    print('[{:.8f}] boot: loading convert'.format(time.time() - starttime))
    from imports import convert
    n.write('[{:.8f}] boot: imports.convert loaded\n'.format(time.time() - starttime))
    print('[{:.8f}] boot: loading helper'.format(time.time() - starttime))
    from imports import helper
    n.write('[{:.8f}] boot: imports.helper loaded\n'.format(time.time() - starttime))
                ...
                if x in cmdlist:
                    if waiting == 0:
                        if x == 'ls':
                            ...
                            if _user == 'root':
                                print(helper.helper('y'))
                            else:
                                print(helper.helper('n'))
                        elif x == 'cl':
                            tmp = subprocess.call(term, shell=True)
                        elif x == 'rb':
