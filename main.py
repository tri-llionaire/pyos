...
_build = '314'
try:
    print('[001] [0.00000000] boot: loading time')
    import time
    starttime = time.time()
    print('[002] [{:.8f}] boot: loading platform'.format(time.time() - starttime))
    import platform
    print('[003] [{:.8f}] boot: loading terminal commands'.format(time.time() - starttime))
    red = ''
    green = ''
    yellow = ''
    blue = ''
    ...
    if platform.system() == 'Linux':
        ...
        white = '\033[0m'
        term = 'clear'
    print('[004] [{:.8f}] boot: on {}{}{}{}{}'.format(time.time() - starttime, platform.system(), platform.release(), platform.version(), platform.machine(), platform.node()))
    print('[005] [{:.8f}] boot: loading subprocess'.format(time.time() - starttime))
    import subprocess
    ...
    cmdlist = ['start', 'exit', 'rb', 'cd', 'md', 'ls', 'pd', 'cf', 'cl', 'help', 'cotw', 'scan', 'hash', 'cat', 'edit', 'del', 'news', 'time', 'weather', 'pystocks', 'p', 'chusr', 'rbu']
    filedict = {}
    waiting = 0
    addi = ''
    added = []
    ...
    print('pyos 3.4.5.2 cold cow {}.058.077 08.22.19 @tri-llionaire with @dentafrice, @Akuhcap'.format(_build))
